...
// Updated Student Node Component with better text formatting
const StudentNode = ({ data, position, onMove, onRemove, zoom, isSelected, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      {/* Tooltip - Hide during PDF export */}
      {showTooltip && !isDragging && (
        <div className="absolute z-50 bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-80 bg-white rounded-lg shadow-xl border border-apple-200 p-4 print:hidden">
          ...
          <div className="space-y-2 text-sm">
            ...
            <p><span className="font-medium">Major:</span> {data.major}</p>
            ...
            {data.about_me && (
              <p><span className="font-medium">About:</span> {data.about_me}</p>
            )}
            ...
            {data.professor_notes && data.professor_notes.length > 0 && (
              <div className="bg-yellow-50 rounded-lg p-2 mt-2">
                ...
                {data.professor_notes.map((note, index) => (
                  <p key={index} className="text-yellow-700 text-xs mt-1">
                    {note.notes}
                  </p>
                ))}
              </div>
          ...
          <div className="mt-2 text-xs text-apple-500">
